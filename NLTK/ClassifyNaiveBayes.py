...
@author: Dinesh
Classifying based on trained Naive Bayes Model
...
f = open('NaiveBayes_FullSet.pickle', 'rb')
    ...
    if len(row)>1:
        ...
        if tweet is None:
            tweet=""
                ...
                result = NBClassifier.classify(extract_features(getFeatureVector(processedTestTweet,stopWords)))
                #print(result)
                if result == '"positive"':
